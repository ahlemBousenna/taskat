...
-- 24. ترقية استوديو بارون: نوع المحتوى (فيديو طويل / ريلز) ومدة المحتوى النهائي بالثواني
alter table public.youtube_videos add column if not exists content_type text not null default 'video';
alter table public.youtube_videos drop constraint if exists youtube_videos_content_type_check;
alter table public.youtube_videos add constraint youtube_videos_content_type_check check (content_type in ('video', 'reel'));
